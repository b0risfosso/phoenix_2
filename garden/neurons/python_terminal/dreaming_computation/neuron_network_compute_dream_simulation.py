#!/usr/bin/env python3
"""
Neuron Network Computation and Dream Simulation
-----------------------------------------------
...
Run:
    python neuron_network_compute_dream_simulation.py
...
class NetworkParams:
    ...
    learned_weight_scale_na: float = 0.18
    learning_rate: float = 0.025
    dream_external_silence: float = 0.35
    dream_replay_boost_na: float = 0.38
    dream_recombination_na: float = 0.10
...
def pattern_current_na(t_ms: float, neuron_index: int, p: NetworkParams, plan: RoundPlan) -> float:
    ...
    if plan.pattern_label == "A":
        ...
        if neuron_index == 1 and in_window_2:
            current += p.pattern_drive_na * 0.85
        if neuron_index == OUTPUT_A_INDEX and 180.0 <= t_ms <= 260.0:
            current += p.output_gate_na
    elif plan.pattern_label == "B":
        if neuron_index in (2, 3) and in_window_1:
            current += p.pattern_drive_na
        if neuron_index == 2 and in_window_2:
            current += p.pattern_drive_na * 0.85
        if neuron_index == OUTPUT_B_INDEX and 180.0 <= t_ms <= 260.0:
            current += p.output_gate_na
...
def update_memory(memory: NetworkMemory, trace: MemoryTrace, p: NetworkParams) -> None:
    ...
    for i in range(NETWORK_SIZE):
        for j in range(NETWORK_SIZE):
            memory.learned_weights[i][j] = clamp(memory.learned_weights[i][j], -0.4, 1.2)
...
def ai_controller(previous_params: NetworkParams, result: Dict[str, float | str | bool]) -> NetworkParams:
    ...
    if mode == "compute":
        ...
        if not correct:
            decisions.append("Computation was wrong or uncertain; increase pattern drive and learned association strength.")
            new_p.pattern_drive_na += 0.045
            new_p.output_gate_na += 0.015
            new_p.learned_weight_scale_na += 0.015
            new_p.v_threshold_mv -= 0.15
        else:
            decisions.append("Computation succeeded; slightly increase learning and preserve the current decision pathway.")
            new_p.learning_rate += 0.002
            new_p.synaptic_decay_ms += 0.25

        if abs(output_a - output_b) < 2:
            decisions.append("Output neurons were close; sharpen classification margin with stronger pattern drive.")
            new_p.pattern_drive_na += 0.025
    else:
    ...
    target_total_spikes = 34
    if total_spikes < target_total_spikes - 12:
        ...
        decisions.append("Network was overactive; increase inhibition and spike adaptation.")
        new_p.base_current_na -= 0.05
        new_p.inhibitory_weight_na += 0.035
        new_p.spike_adaptation_add_na += 0.01
        new_p.v_threshold_mv += 0.20
    else:
    ...
    new_p.output_gate_na = clamp(new_p.output_gate_na, 0.0, 0.7)
    ...
    new_p.learned_weight_scale_na = clamp(new_p.learned_weight_scale_na, 0.0, 0.7)
    new_p.learning_rate = clamp(new_p.learning_rate, 0.0, 0.12)
...
def main() -> None:
    print("Neuron Network Computation and Dream Simulation")
