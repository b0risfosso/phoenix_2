#!/usr/bin/env python3
"""
DummyJSON + Simple Flame Combustion API Driver - Sustain Mode
----------------------------------------------
...
Run:
    python flame_dummyjson_api_driver_sustain.py
...
def should_use_sustain_mode(product: Dict[str, Any], round_number: int) -> bool:
    """
    Decide whether a DummyJSON product should create a sustained flame.

    Sustain mode is enabled for:
    - highly rated products
    - higher-priced products
    - healthy stock levels
    - every third round, so the run includes controlled sustain tests
    """
    price = safe_float(product, "price", 0.0)
    rating = safe_float(product, "rating", 0.0)
    stock = safe_float(product, "stock", 0.0)

    return rating >= 4.25 or price >= 75.0 or stock >= 85.0 or round_number % 3 == 0
...
def apply_sustain_flame_boost(
    ...
    *,
    ...
    """
    Modify FlameParams in-place to create longer burns.

    The original model cools very aggressively. This mode increases available
    heat generation and lowers heat losses so API differences remain visible
    after ignition instead of every round ending after 0.04 seconds.
    """
    ...
    # More reactant availability.
    p.initial_fuel = flame.clamp(p.initial_fuel + 0.22 + 0.16 * price_score, 0.05, 2.0)
    p.initial_oxygen = flame.clamp(p.initial_oxygen + 0.35 + 0.28 * stock_score, 0.05, 2.0)
    p.fuel_supply_rate = flame.clamp(p.fuel_supply_rate + 0.06 + 0.05 * price_score, 0.0, 0.5)
    p.oxygen_supply_rate = flame.clamp(p.oxygen_supply_rate + 0.28 + 0.18 * rating_score, 0.0, 1.4)

    # Stronger heat generation, but still bounded.
    p.spark_energy_J = flame.clamp(p.spark_energy_J + 120.0 + 80.0 * rating_score, 0.0, 1200.0)
    p.reaction_strength = flame.clamp(p.reaction_strength + 1.05 + 0.35 * rating_score, 0.1, 6.0)
    p.heat_of_combustion_J_per_unit = flame.clamp(
        900.0 + 260.0 * rating_score + 80.0 * title_score,
        520.0,
        1500.0,
    )
    p.heat_capacity_J_per_K = flame.clamp(0.72 - 0.05 * rating_score, 0.45, 1.20)

    # Lower losses; this is the main sustain change.
    p.convective_loss_coeff = flame.clamp(p.convective_loss_coeff * 0.34, 0.05, 3.0)
    p.radiative_loss_coeff = flame.clamp(p.radiative_loss_coeff * 0.10, 0.0, 2.6e-8)
    p.mixing_loss_coeff = flame.clamp(p.mixing_loss_coeff * 0.42, 0.0, 0.8)

    # Make extinction less abrupt.
    p.ignition_temperature_K = 690.0
    p.extinction_temperature_K = 555.0

    # Product profile still matters.
    if profile == "fuel_rich":
        # Fuel-rich sustained flames remain smoky, but not automatically starved.
        p.oxygen_supply_rate = flame.clamp(p.oxygen_supply_rate + 0.18, 0.0, 1.4)
        p.humidity = flame.clamp(p.humidity * 0.75, 0.0, 1.0)
        p.soot_rate = flame.clamp(p.soot_rate * (0.75 + 0.25 * discount_score), 0.0, 0.12)
    elif profile == "hot":
        p.reaction_strength = flame.clamp(p.reaction_strength + 0.35, 0.1, 6.0)
        p.convective_loss_coeff = flame.clamp(p.convective_loss_coeff * 0.90, 0.05, 3.0)
    elif profile == "cooling":
        p.convective_loss_coeff = flame.clamp(p.convective_loss_coeff * 1.25, 0.05, 3.0)

    # Smaller chamber concentrates energy enough to keep reaction going.
    p.chamber_volume = flame.clamp(p.chamber_volume * 0.74, 0.3, 3.0)
...
def map_product_to_flame_params(
    ...
    if should_use_sustain_mode(product, round_number):
        p.fuel_name = f"DummyJSON sustain {profile}: {title[:22]}"
        apply_sustain_flame_boost(p, product=product, profile=profile, title_score=title_score)
    else:
        p.fuel_name = f"DummyJSON brief {profile}: {title[:24]}"
...
def print_api_items(products: List[Dict[str, Any]]) -> None:
    ...
    for index, product in enumerate(products, start=1):
        ...
        profile = category_flame_profile(category)
        sustain = should_use_sustain_mode(product, index)
        print(
            f"{index:>2}. id={product.get('id')!s:<3} "
            f"profile={profile:<12} "
            f"sustain={str(sustain):<5} "
...
def main() -> None:
    ...
    print("\nDummyJSON-driven Simple Flame Combustion Simulation with Sustain Mode")
    print("Each DummyJSON product is converted into flame fuel, oxygen, spark, cooling, humidity, soot, chamber, and sustain parameters.")
